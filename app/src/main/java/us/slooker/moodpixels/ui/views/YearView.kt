...
class YearView @JvmOverloads constructor(
    ...
    private val monthLabelPaint = android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG).apply {
        color = android.graphics.Color.parseColor("#555555")
        isFakeBoldText = true
        style = android.graphics.Paint.Style.FILL
    }

    private val yearDayHeaderPaint = android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG).apply {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (monthIdx in 0..11) {
            ...
            listOf("M","T","W","T","F","S","S").forEachIndexed { i, d ->
                canvas.drawText(d, bx + dpToPx(2f) + i * dayCellW + dayCellW / 2f - yearDayHeaderPaint.measureText(d) / 2f,
                    by + labelH + dpToPx(11f), yearDayHeaderPaint)
            }
            ...
            for (d in 1..daysInMonth) {
                ...
                if (!dayEntries.isNullOrEmpty()) {
                    val dominantColor = dayEntries.values
                        .groupBy { it.colorValue }
                        .maxByOrNull { it.value.size }!!
                        .key
                    cellPaint.color = dominantColor
                } else {
                    cellPaint.color = if (dayDate == today) Color.parseColor("#FFF9C4")
            ...
            // Month block border
            gridPaint.color = Color.parseColor("#CCCCCC")
